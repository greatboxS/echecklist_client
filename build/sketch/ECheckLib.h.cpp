#line 1 "e:\\Visual Code\\EcheckList_Stm32\\ECheckLib.h"
/*
 Name:		ECheckLib.h
...
#define TIM_500MS 500000
#define TIM_200MS 200000
#define TIM_100MS 100000
#define TIM_50MS 50000
...
typedef struct ECheckStation_t
{
	...
	void EEPROM_save_area_id()
	{
		...
		EEPROM.write(EEPROM_AREA_ID, AreaId);
		EEPROM.end();
	}
	...
	void EEPROM_save_person_id()
	{
		...
		EEPROM.begin();
		EEPROM.write(EEPROM_PERSON_ID, CheckPerson_Index);
		EEPROM.end();
	}
	...
	void EEPROM_save_last_check_state()
	{
		...
		for (size_t i = 0; i < ECheckItems.size(); i++)
		{
			EEPROM.write(EEPROM_ITEM_STATE_ID + i, ECheckItems[i].Status);
		}
		EEPROM.end();
	}
	...
	void EEPROM_read_local_data()
	{
		...
		EEPROM.begin();
		uint8_t area_id = EEPROM.read(EEPROM_AREA_ID);
		uint8_t person_id = EEPROM.read(EEPROM_PERSON_ID);
		...
		{
			area_id = 1;
			EEPROM.write(EEPROM_AREA_ID, area_id);
		}

		if (person_id > 10)
		{
			person_id = 1;
			EEPROM.write(EEPROM_PERSON_ID, person_id);
		}
		...
		for (size_t i = 0; i < ECheckItems.size(); i++)
		{
			ECheckItems[i].Status = EEPROM.read(EEPROM_ITEM_STATE_ID + i);
		}
...
#define START_PACKAGE 0xF0
#define END_PACKAGE 0x0F
...
#line 1 "e:\\Visual Code\\EcheckList_Stm32\\ECheckList_Client.ino"
// date : 27/2/2020
